...
class ChannelRouter:
    ...
    def _get_sgp_picks(self, recommendations: List[dict], home_team: str = "HOME", away_team: str = "AWAY") -> List[dict]:
        ...
        Filters out correlated picks (ML + spread from same team).
        When both exist for the same team, keeps the one with greater edge.
        """
        ...
        # Group picks by team to detect ML + spread correlation
        # Picks are: total (independent), spread (team-based), ml (team-based), team_total (team-based)
    ...
    def _format_sgp(self, prediction: dict, sgp_picks: List[dict]) -> str:
        ...
        lines.append("")
        lines.append("_PerryPicks REPTAR Model_")
